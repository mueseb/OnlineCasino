...
public class CardResource {
    ...
    @Produces
    @Consumes
    public List<DBgame> getPlayerGames(String name)
    {
        ...
        try
        {
            gameList = blackjackDB.getGamesByUserID(blackjackDB.getUserIdByName(name));

        }catch (SQLException e) {
            throw new RuntimeException(e);
        }
        for (int i = 0; i < gameList.size(); i++) {
            System.out.println(gameList.get(i).getPlayerHand());
        }
...
//        return Response.ok(getPlayerCards(table.getPlayer())).build();
    ...
    public String getDealerStarterCards()
    {
        ...
        return getDealerHiddenCards(table.getDealer());
    }
    ...
    public String getDealerHiddenCards(BlackJackDealer dealer){
        String cards="F02;";

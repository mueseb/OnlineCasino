...
//        }
    ...
    public List<DBgame> getGamesByUserID(int id) throws SQLException {
        ...
        while(rs.next())
        {
            gameList.add(new DBgame(rs.getInt("id"), rs.getInt("playerid"),  rs.getInt("bet"), rs.getString("dealerhand"), rs.getString("playerhand"), rs.getTimestamp("starttime"), rs.getTimestamp("endtime"), rs.getString("result")));
        }
        return gameList;
    }
